...
# Load the trained model safely (no re-training)
try:
    # try normal load (will attempt to deserialize compile config)
    model = tf.keras.models.load_model('modell.h5')
except Exception:
    # fallback: load without compiling to skip incompatible serialized loss/optimizer
    model = tf.keras.models.load_model('modell.h5', compile=False)

    # Manually compile for inference (this DOES NOT re-train the model)
    # If you used a different optimizer/metrics during training, replace accordingly.
    model.compile(
...
# one-hot encode geography and combine
geo_encoded = one_hot_encoder.transform([[geography]]).toarray()
geo_df = pd.DataFrame(geo_encoded, columns=one_hot_encoder.get_feature_names_out(['Geography']))
...
# prediction
prediction = model.predict(input_scaled)
# model.predict for a binary output usually returns shape (1,1)
